...
- 1つの鎖で、各プロパティを束縛できるのは1回までです。経路はすべて数えます
...
コンテキストを捨てて束縛します。角括弧の中には、コンテキストを読むための名前が無いからです。
...
から束縛すると [BCF3007](./diagnostics.md#bcf3007) です。
...
`.Param` の規則は、セレクターを書いた場所で検査します。だから [BCF3006](./diagnostics.md#bcf3006) と BCF3007 は、呼び出す
...
ません（後述の [BCF1002](./diagnostics.md#bcf1002)）。だからコンポーネントのライブラリが配れるのはコンポーネントまでで、
