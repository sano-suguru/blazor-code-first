...
同じゲッターを読む診断がもう1つあります。BCF1002 です。生成コードが名指せないものを式が参照して
いると、これを報告します。名指せないのは、ローカル変数、ローカル関数、範囲変数、ラベルのうち、
宣言が参照の傍らまで生成コードへ運ばれないものです。よくある入り口は、式のある部分で宣言して別
の部分から読む書き方です。ジェネレーターは要素の各部を、書かれた順に出力しないからです。
...
`If` の条件は両方の分岐を包み、`ForEach` のソースはコンテンツとキーを包みます。それ以外の
位置では、上の `greeting` と同じように、`return` の手前の文でローカル変数を宣言してください。
