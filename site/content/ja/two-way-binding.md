...
意識して選ぶべき帰結が1つあります。`"oninput"` では巻き戻しがキーストロークごとに走ります。
`int` の束縛に `4.5` と打つと、手元に残るのは `45` です。`.` が拒否されて消え、そのあとに `5` が
届くからです。空文字列も解析できないので、欄を空にすることもできません。空にすると前の値がその
まま戻ります。数値の入力では、たいていこれは望みではありません。
