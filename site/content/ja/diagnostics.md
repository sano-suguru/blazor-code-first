...
`switch` は、それぞれ自分用の複製を必要とします。

内容が受け付けるのは、式のラムダ、末尾に `return` を1つ持つブロック、末尾がC#本来の `if`/`else`
であるブロック([BCF2002](#bcf2002))、引数1つの `View` を返すメソッドグループです。`switch` と
`foreach` は受け付けません。
...
Info. C# 本来の `if`/`else` は、動的なリージョンへ縮退します。
...
`if`/`else` は、境界のシーケンスが構文上の位置に固定されたリージョンへ、そのまま丸ごと移されます。
これは `If()` と同じ形です。`If()` と違い、各分岐の内容は静的に割り当てられたシーケンス範囲ではなく、
その場で合成された `RenderFragment` を通して描かれます。実行される分岐は常に1つだけなので、
実行されないかもしれない内容へ静的な幅を確保できないからです。正しさは変わりません。実行された
分岐のフレームは、静的なテンプレートと比較されるのではなく、作り直されます。

`if`/`else` の連鎖1つにつき、最も外側の `if` で1回だけ報告されます。`else if` の連結数がいくつでも
同じです。`switch` と `foreach` はこの位置ではまだ受け付けません。今も
