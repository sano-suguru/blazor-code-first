...
title: 診断
order: 70
source-hash: 2ba97b14
---

このコンパイラが報告する診断のすべてと、その意味と、代わりに書くものです。ガイドは、診断が出て
きた場所で説明する代わりに、ここへリンクします。
...
修飾子が要るのは、オーバーライドを宣言するクラスだけです。BlazorCodeFirst の基底を継承している
...
せん。CS0534 が名指しするのは足りないメンバーで、修飾子ではありません。
...
宣言を運べる位置は2つあります。どちらも生成コードでは、それを読む側を包むヘッダになるからです。
`If` の条件は両方の分岐を包み、`ForEach` のソースはコンテンツとキーを包みます。

BCF1003 との違いはここにあります。BCF1003 は式を順序付けできなかったという意味で、BCF1002 は
順序付けはできたが、生成ファイルから見えないものを参照したという意味です。

### BCF1003

Error. 式はモデルの段階まで届きましたが、ジェネレーターが解析しない構文を使っているため変換
できませんでした。
...
その `return` の手前には、ローカル変数の宣言と式文を置けます。書いた文は生成された `RenderView`
のフレーム発行の手前へ移植されます。
...
ティには、そもそもゲッターの本体がありません。本体がどうしてもこの形にならないなら、`RenderView`
を手で書いてください。そのとき設計時の式は使われなくなり、何も報告されません。

BCF1004 が咎めるのは宣言そのものです。1つのクラスが `partial` の欠落とゲッターの問題を同時に
抱えることはあり、知らされるのは一度に1つです。`partial` の検査が先に走るので、まず BCF1001 だけ
が出ます。
...
コンポーネントをトップレベルの型へ移してください。生成コードは、外側の型宣言の連なりを、その型
引数まで含めて再現できません。この診断がなければ、入れ子であることは CS0534 としてしか現れま
せん。
...
ジェネレーターが展開するのは、読めるものだけです。設計時の表層と、このコンパイルの中で宣言された
`[ViewPart]` メソッドです。読めない呼び出しも正しく、正しく描画されます。その領域のフレームが、
静的なテンプレートと差分を取る代わりに、作り直されるというだけです。

参照プロジェクトや NuGet パッケージの `[ViewPart]` はここに来ます。定義は現コンパイルの構文から
集められ、IL は本体の構文を持たないからです。アセンブリを越えた再利用はコンポーネントの仕事です。
...
`Html.Div`、`.Class(...)`、`.OnClick(...)` をはじめ、要素のファクトリと装飾は、すべて中身があり
ません。`View` は空の構造体で、要素ヘルパーは何も返さず、装飾はレシーバーをそのまま返すだけです。
ジェネレーターが読むのは書かれた *構文* であって、値ではありません。読む場所も3つしかありません。
コンポーネントの `Body`、レイアウトの `Chrome`、そして `[ViewPart]` メソッドの本体です。

同じ API はどこからでも呼べて、その3つ以外では何の意味も持ちません。コンパイルは通り、何かを
組み立てたように見えて、何も起きません。

設計時型の値をフィールドやプロパティに取っておくのは咎めません。報告するのは、ローカル変数、
破棄、引数の場合だけです。

### BCF3030

Error. 呼び出し先は設計時の表層から `View` を組み立てているのに `[ViewPart]` を持たないので、
この呼び出しは何も描画しません。
...
これは BCF3029 を呼び出しの反対側から見たものです。BCF3029 は誰も読まない場所に書かれた設計時の
構文を、こちらは誰も読まなかった設計時の構文を書いた呼び出し先を報告します。
...
Error. 設計時の値の式にある型名が解決できず、その綴りが元ファイルの字句的な文脈に依存しています。
...
Error. `Element` のタグが、タグ名として綴られたコンパイル時定数ではありません。
...
問題でもありません。ただ、書いた場所で要素が自分の名前を名乗らなくなります。

綴りを求めるのは翻訳の破れだからです。どの要素も名乗れないタグは、二通りに描かれます。プリレン
ダリングはマークアップに書き、HTML パーサーがそれを解釈し直します。対話的描画は `createElement`
に渡し、それが拒んで回路ごと落とします。
...
子は HTML を往復して生き残りません。プリレンダリングはパーサーが受け付けない終了タグを書き出し、
パーサーは子を要素の外へ押し出して兄弟にします。孤立した `</br>` は開始タグとして読み直されるので、
`Br["x"]` は `<br>` 2つになります。対話的描画にはパーサーが挟まらず、同じ子を要素の中に置きます。
1つの式から2つの DOM ができ、ハイドレーションでページの形が変わります。
...
これがこの表層が HTML について検査する限界で、限界は意図的です。BCF3016 は要素のタグだけから
決定できます。ある子がある親の中に置けるかは決定できないので、`Table[Div["x"]]` も、要素が定義
しない属性（`Div.Href("/x")`）も受け付けます。

### BCF3027

Error. 要素ヘルパーが名乗るはずの単純名を、自分の宣言が取っています。
...
そしてそのどれも目に入りません。本体が変換されない限りコンポーネントには生成された `RenderView`
がなく、コンパイラはメソッド本体を束縛する前に止まるからです。4つはすべてそこで見つかります。
...
装飾は持ち主の要素の属性へ畳まれるので、付く先の要素が要ります。`If`、`ForEach`、`Fragment`、
...
綴り間違いも、要素を取って要素を返す自作の拡張メソッドも、ここに来ます。綴り間違いには C# の
エラーがありますが、同じ宣言段階の停止が、それを目に入らなくします。

### BCF3010

Error. 1つの要素で、同じ属性またはイベントを2回以上束縛しています。
...
属性チャネルで2回束縛すると、後の書き込みが勝って先のものが死にます。1つの名前を属性チャネルと
イベントチャネルで1回ずつ束縛すると両方が残り、インラインのハンドラーと C# のハンドラーがイベント
ごとに両方走ります。どちらも書いたとおりではありません。
...
名前はリテラルへ落とされます。定数であることは、class の畳み込みと二重束縛の検出の前提でもあり
ます。
...
class チャネルは、書かれた装飾を1つの値へテキストとして連結します。だから `class` が取るのは
...
同じ `true` が `class="a True"` になります。1つの綴りが、鎖の別の場所に書かれた個数によって
二通りの意味を持つ、というのが問題です。
...
して先勝ちになり、対話的描画は DOM へ適用するので後勝ちになります。

class の値は1か所から与えてください。束縛のゲッターにすべてを持たせるか、束縛をやめて装飾だけに
します。
...
`.Key` とその仲間は、値を1つだけ持つチャネルを占めます。3つとも壊れ方が違い、どれも目に見えま
せん。`SetKey` は開いているフレームへ書き込むので2回目が1回目を上書きします。
`AddComponentRenderMode` は追記され、レンダラーは最初に見つけたフレームを読むので、そこでは
2つ目が死にます。参照の捕捉も追記され、両方の動作が走ります。

### BCF3034

Error. コンポーネント自身の宣言がレンダーモードを固定しているので、呼び出しサイトからは設定
できません。
...
を受け取ると、`ComponentFactory` が例外を投げます。呼び出しサイトの形は、自分ではモードを宣言
しないコンポーネントのためにあります。同じコンポーネントを、あるページからは対話的に、別のページ
からは静的に描く場合です。
...
ふつうの属性として `AddAttribute` に届くので、ハンドラーは呼ばれず、実行時にも何も報告されません。

`.Bind` では、これがもう1つの仕事をします。属性名とイベント名は隣り合う文字列引数で、入れ替えても
コンパイルは通ります。入れ替えを止めるのがこの検査です。

### BCF3028

Error. ハンドラーの引数型を、名指ししたイベントは配れません。

```csharp
Button.On("onclick", (MouseEventArgs e) => Zoom(e.ClientX, e.ClientY))["Zoom"]   // 配られる型
Button.On("onclick", (EventArgs e) => Save())["Save"]                            // その基底: これでよい
Button.On("onclick", (KeyboardEventArgs e) => Save())["Save"]                    // BCF3028
```

Blazor はイベントの引数オブジェクトをハンドラーの引数型へキャストして配るので、配られる型の基底は
受け取れて、兄弟の型は受け取れません。`EventArgs` ですらない型も同じ診断です。

対応表はフレームワークが配る `[EventHandler]` のメタデータと、ビルド中のコンパイルにある登録から
読みます。登録のないイベントには対応表がないので検査しません。
...
`.PreventDefault` と `.StopPropagation` は、手前に書かれたイベントに付きます。鎖が示す読み方は
...
モデルがどちらへ倒れても一方は死に、どちらが死ぬかは呼び出しサイトから見えません。
...
Error. `ForEach` の中身の根が単一の要素でもコンポーネントでもないので、キーの付く先がありません。
...
キーは中身の根のフレームに付き、`SetKey` は現在開いている要素かコンポーネントのフレームに付きます。
裸の `If`、入れ子の `ForEach`、裸のテキスト、`Fragment`、`Raw`、外から渡された `RenderFragment`
は、いずれもキーの付く単一のフレームを開きません。

中身を要素で包んでください。`key: null` でキーを降りた `ForEach` は `SetKey` を出さないので、
それらの根も置けます。

### BCF3004

Error. `ForEach` のキーか中身が、ジェネレーターの順序付けできない形です。

キーの本体は `SetKey` へ移植されるので、式である必要があります。中身には静的なシーケンス空間が
1つ与えられ、それを毎回の繰り返しが使い回します。2つ目の `return` やネイティブの制御文は、それ
ぞれ自分用の複製を要します。

中身が受け付けるのは、式のラムダ、末尾に `return` を1つ持つブロック、引数1つの `View` を返す
メソッドグループです。

### BCF3032

Error. `ForEach` がキーを付ける中身の根が、自分でも `.Key` を書いています。

```csharp
ForEach(rows, key: r => r.Id, content: r => Li.Key(r.Id)[r.Name])   // BCF3032
```

1つのフレームに `SetKey` が2回届き、後のほうが勝ちます。だからどちらのキーが有効かは、呼び出し
サイトではなくフレームの発行順で決まります。根に付けるか、ループに付けるか、どちらかにしてくだ
さい。
...
ジェネレーターはセレクターの綴りからパラメーター名を読むので、キャスト、メソッド呼び出し、
...
束縛できるのは、`[Parameter]` が付いていてアクセス可能なセッターを持つプロパティだけです。それ
以外を設定すると、Blazor がパラメーターを適用するときに例外になります。

### BCF3007

Error. 1つの鎖が、同じパラメーターを2回以上束縛しています。
...
Blazor は最後の書き込みを適用するので、先の値は死にます。

### BCF3012

Error. ジェネレーターが走った時点で型引数が解決できませんでした。

よくある原因は、同じプロジェクトで宣言された `.razor` コンポーネントです。Razor コンパイラ自身が
ソースジェネレーターであり、ソースジェネレーターは互いの出力を観測できません。だから最終的な
コンパイルに存在していても、ここでは解決できません。

参照プロジェクトへ移すか、手書きの C# コンポーネントにするか、名前を直してください。参照プロ
ジェクトや NuGet パッケージの同じコンポーネントはふつうに解決します。

綴り間違い、アクセスできない型、あいまいな名前、`using` の不足もここに来ます。その場合は同じ位置
に C# の解決エラーも出ます。
...
角括弧は `ChildContent` という名前のパラメーターに束縛されます。Razor が入れ子の内容を落とす先と
同じです。そのパラメーターは、設定できる `[Parameter]` で、型が `RenderFragment` または
`RenderFragment<TContext>` である必要があります。ジェネリックのほうは、コンテキストを捨てて子を
受け取ります。

### BCF3014

Error. 中身のない設計時の値を、ジェネリックの `Param` に渡しています。
...
ジェネリックの `Param` は値の式をそのまま出すので、これを束縛すると目印が代入されます。`object`
...
メソッドグループ、匿名メソッド、本体が文のラムダは、いずれも内容を呼び出しの向こうへ隠します。
順序付ける式も、生成したコンテキスト変数を差し替える引数のシンボルも残りません。

### BCF3025

Error. 内容を取る `[ViewPart]` の本体の外に `Slot` があるか、ちょうど1回以外の回数で名指しして
います。
...
`Slot` は、呼び出し側が角括弧で渡した内容を部品が置く位置を示します。だから置く内容がない場所では
意味を持ちません。コンポーネントの `Body` や `Chrome` は角括弧を受け取らず、`View` を返す部品は
角括弧なしで呼ばれます。

内容を取る部品は、返り値の型に `SlotView` を宣言し、`Slot` をちょうど1回名指しします。2回書けば
...
ゲッターの本体は2回移植されます。束縛される属性の値として1回、バインダーの現在値として1回です。
だから式として取り出せる必要があります。本体が文のラムダとメソッドグループは、どちらもそれを
呼び出しの向こうへ隠します。
...
ゲッターのみの形は、ゲッターの本体を代入の左辺に置いてセッターを導きます。だからその本体は、
フィールド、設定できるプロパティ、セッターを持つインデクサーへの要素アクセスのいずれかである必要
があります。
...
Error. コンポーネントに対応する変更コールバックがないので、双方向の束縛に書き戻す先がありません。
...
コンポーネントの束縛は、パラメーター名を書かせるのではなく導きます。要素の表層とは逆です。それが
成り立つのは、導出を検査できるからです。コンポーネントの型は分かっているので、ジェネレーターは
導いた `{名前}Changed` を引き、無いか型が違えばこれを報告します。

要素の束縛にはこの検査がありません。だから名前を2つとも書かせます。
...
`BindConverter.FormatValue` と `CreateBinder` が書式を取るオーバーロードを宣言しているのは、
`DateTime`、`DateTimeOffset`、`DateOnly`、`TimeOnly` と、それらの null 許容形だけです。それ以外に
書式を書くと、生成ファイルに束縛できない呼び出しが残ります。その C# のエラーは生成コードの中で
起きるので、目に入りません。

書式を外すか、ゲッターで整形してセッターで解析してください。受け付ける集合はフレームワーク自身の
メタデータから読んでおり、このコンパイラが列挙しているのではありません。カルチャは問題になりま
せん。この表層が束縛するどの型もカルチャを持てます。
