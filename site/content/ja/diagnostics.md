...
スプライスされた射影(`.. source.Select(item => …)`)は、BCF1004 のゲッターと同じように、
本体を作者自身の名前のまま移植します。そのため [BCF1004](#bcf1004) が述べる予約名の規則が、
ここにも届きます。
...
2つ目の `return` は専用のシーケンス空間を必要とするため受け付けません。自動プロパティにも、
そもそもゲッターの本体がないので受け付けません。C# 本来の `if`/`else` や `switch` なら、ゲッター
の末尾に置けます([BCF2002](#bcf2002))。C# 本来の `foreach` だけは置けません。プロパティのゲッター
は C# のイテレータブロックになれないので、この診断とは関係なく `yield return` がそこでは無効だか
らです。同じ形は `[ViewPart]` メソッドでイテレータとしてなら受け付けます
([制御構文](./control-flow.md#viewpart-でイテレートする))。ゲッターと違い、メソッドはイテレータに
なれるからです。`__builder` という名前のローカル、または `__bcf_` で始まるローカルも、ここには
宣言できません。この2つの綴りは、移植したローカルが着地するどの位置でもジェネレーターが予約して
います。名前を変えてください。本体がどうしてもこの形にならないなら、`RenderView` を手で書いて
ください。そのとき設計時の式は使われなくなり、何も報告されません。
...
Error. 要素タグエイリアスとして使われた `static ElementView` プロパティが、参照アセンブリ側で
宣言されています。

```csharp
// 参照先プロジェクトや NuGet パッケージの中:
...
ジェネレーターはエイリアスの宣言自体の構文 —— `Element("my-card")` —— を読んでタグを見つけます。
IL は本体を持たないので、参照アセンブリを経由した宣言には読み取るものがありません。エイリアスは
このコンパイル内で宣言してください。

```csharp
static ElementView MyCard => Element("my-card");

MyCard["content"]     // これでよい
```

`[ViewPart]` も同じ理由で同じ壁に当たります（[BCF1002](#bcf1002)）。どちらの機能も対象の
ソースではなく自分自身のソースを読むので、両方がコンパイル境界で止まります。
...
シーケンス空間が1つ与えられ、それを毎回の繰り返しが使い回します。2つ目の `return` は自分用の
複製を必要とするので受け付けません。

内容が受け付けるのは、式のラムダ、末尾に `return` を1つ持つブロック、末尾がC#本来の `if`/`else`
であるブロック([BCF2002](#bcf2002))、末尾がC#本来の `switch` であるブロック
([BCF2002](#bcf2002))、引数1つの `View` を返すメソッドグループです。C# 本来の `foreach` もここで
は受け付けませんが、理由は別です。`content` は常にラムダで、ラムダは C# のイテレータブロックに
なれないので、この診断に届く前に `yield return` がすでに無効です。同じ形は `[ViewPart]` メソッド
でイテレータとしてなら受け付けます([制御構文](./control-flow.md#viewpart-でイテレートする))。
...
Info. C# 本来の `if`/`else` や `switch` は、動的なリージョンへ縮退します。
...
protected override View Body
{
    get
    {
        if (_flag)                 // BCF2002: 正しいが、静的には割り当てられない
        {
            return Span["yes"];
        }
        else
...
`Body`/`Chrome` ゲッター、`ForEach` の内容ラムダ、`[ViewPart]` の本体の末尾に書いたC#本来の
`if`/`else` や `switch` は、境界のシーケンスが構文上の位置に固定されたリージョンへ、そのまま丸ごと
移されます。これは `If()` と同じ形です。`If()` と違い、各分岐・各セクションの内容は静的に割り当て
られたシーケンス範囲ではなく、その場で合成された `RenderFragment` を通して描かれます。実行される
分岐・セクションは常に1つだけなので、実行されないかもしれない内容へ静的な幅を確保できないからです。
正しさは変わりません。実行された側のフレームは、静的なテンプレートと比較されるのではなく、
作り直されます。

`if`/`else` の連鎖1つ、または `switch` 1つにつき、最も外側の `if` か `switch` の判定式で1回だけ
報告されます。`else if` や `case` の数がいくつでも同じです。`switch` の各セクションを閉じるのは
セクション自身の `return` ですが、生成後のコードでは次のセクションへ流れ落ちないよう、
明示的な `break` も添えられます。

`foreach` はどの位置でもこの診断の対象にはなりません。ゲッターと `ForEach` の内容の位置
([BCF1004](#bcf1004)/[BCF3004](#bcf3004))では、今も受け付けません。これはこの診断の判定によるの
ではなく、プロパティのゲッターもラムダも C# のイテレータブロックになれず、`yield return` がそこ
では無効だからです。`[ViewPart]` メソッドではイテレータとして受け付けます
([制御構文](./control-flow.md#viewpart-でイテレートする))。ただし `if`/`switch` のようには縮退しま
せん。内容は毎回の繰り返しが使い回す静的なシーケンス範囲を1つだけ持ち、`ForEach` の内容と同じ
なので、静的な割り当てを失うことがなく、この診断はそもそも当てはまりません。
...
Error. ループのソース引数が `[ViewPart]` の呼び出しに解決されます。
...
呼び出しが間接的に到達する場合——中間のローカル変数、`.ToList()`、null 許容抑止、括弧、キャスト、
`as` 変換を経由する場合——も同様に報告されます。不活性なサーフェスに対して実行されるという同じ
失敗が、呼び出しがどこに辿り着いても付いて回るためです。

```csharp
var rows = Rows(items);
ForEach(rows, key: item => 0, content: item => item)   // BCF3043
Ul[.. Rows(items)]                                     // 代わりにこう書く（ローカル変数ごと削除する）
```
...
Error. `If` の分岐が、ジェネレーターの順序付けできない形です。

```csharp
If(_flag, delegate() { return Span["yes"]; })   // BCF3044
If(_flag, () => Span["yes"])                    // 代わりにこう書く
```

各分岐が受け付けるのは、式のラムダ、末尾に `return` を1つ持つブロック、末尾がC#本来の `if`/`else`
であるブロック([BCF2002](#bcf2002))、末尾がC#本来の `switch` であるブロック
([BCF2002](#bcf2002))、引数0個の `View` を返すメソッドグループです。これは `ForEach` の内容が
受け付けるのと同じ5つの形です([BCF3004](#bcf3004))。匿名メソッドは対象外です。呼び出し名を
持たないのでメソッドグループとしても読めず、ラムダの形のいずれにも当てはまりません。

分岐の本体にも、ゲッターについて [BCF1004](#bcf1004) が述べる予約名の規則が当てはまります。
