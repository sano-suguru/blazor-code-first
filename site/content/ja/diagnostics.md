...
綴りのほうを求める理由は違います。ここが破れると、翻訳そのものが壊れます。どの要素も名乗れない
タグは、二通りに描かれます。プリレンダリングはマークアップに書き、HTML パーサーがそれを解釈し
直します。対話的レンダリングは `createElement` に渡し、それが拒んで回線ごと落とします。
...
がなく、コンパイラはメソッド本体を解決する前に止まるからです。4つはすべてそこで見つかります。
...
属性チャネルで2回バインドすると、後の書き込みが勝って先のものが死にます。1つの名前を属性チャネル
とイベントチャネルで1回ずつバインドすると、両方が残ります。インラインのハンドラーと C# の
ハンドラーが、イベントごとに両方走ります。どちらも書いたとおりではありません。
...
名前はリテラルへ落とされます。定数であることは、class の畳み込みと二重バインドの検出の前提でも
あります。
...
呼び出しサイトの `.RenderMode` を外し、コンポーネント自身の属性に任せてください。呼び出し側ごと
にモードを変える必要が本当にあるなら、コンポーネントから属性を外します。その場合は、すべての
呼び出しサイトがモードを書くことになります。
...
`.PreventDefault` と `.StopPropagation` は、手前に書いたイベントへ付きます。鎖が示す読み方は
...
修飾子は1回だけ書いてください。値ではなく旗なので、2つ目は1つ目がやったこと以上を何も求めて
いません。
...
Error. `ForEach` の中身の根は、単一の要素かコンポーネントである必要があります。そうでないと、
キーの付く先がありません。
...
それ以外を設定すると、Blazor がパラメーターを適用する時点で例外が飛びます。
...
Error. 1つの鎖が、同じパラメーターを2回以上バインドしています。
...
角括弧は `ChildContent` という名前のパラメーターにバインドされます。Razor が入れ子の内容を落とす
...
`Slot` は、呼び出し側が角括弧で渡した内容をパーツが置く位置を示します。だから置く内容がない場所では
意味を持ちません。コンポーネントの `Body` や `Chrome` は角括弧を受け取らず、`View` を返すパーツは
角括弧なしで呼ばれます。

内容を取るパーツは、返り値の型に `SlotView` を宣言し、`Slot` をちょうど1回名指しします。2回書けば
...
ゲッターの本体は2回移植されます。バインドされる属性の値として1回、バインダーの現在値として1回
です。だから式として取り出せる必要があります。本体が文のラムダとメソッドグループは、どちらも
それを呼び出しの向こうへ隠します。
...
要素のバインドにはこの検査がありません。だから名前を2つとも書かせます。
...
残ります。その C# のエラーは生成コードの中で起きるので、目に入りません。
