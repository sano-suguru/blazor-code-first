...
/// frame. This was written as a prediction of what #140 would produce; since #140 the generator
/// produces that shape itself, and <see cref="StaticHeavyElementView"/> reaches the same frame count.
/// </summary>
/// <remarks>
...
/// <para>
/// The wrapper <c>div</c> stays an element frame here because it has a non-foldable child: a markup
/// frame carries complete markup, so an open tag cannot be folded together with a partial child list.
/// The condition matters — were every child foldable, the wrapper's own open tag would fold with them
/// and the whole body would become one markup frame. Razor keeps the element frame here on the same
/// condition. The two runs are split by the dynamic slot, exactly as Razor splits them.
/// </para>
/// <para>
