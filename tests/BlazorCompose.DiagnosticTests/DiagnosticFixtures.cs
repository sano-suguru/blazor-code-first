...
public sealed class DiagnosticFixtures
{
    ...
    private static string PackRuntime()
    {
        ...
        var (exitCode, output) = RunDotnet(
            [
                "pack",
                Path.Combine(RepoLayout.Root, "src", "BlazorCompose.Runtime", "BlazorCompose.Runtime.csproj"),
                "-c",
                "Release",
                "-o",
                packageDirectory,
        ...
        Assert.True(exitCode == 0, "Packing BlazorCompose.Runtime failed." + Environment.NewLine + output);
