...
- The six excluded groups, and the reason for each, are as follows.
  ...
  5. A foreign vocabulary (`svg` / `math`, and each of their element indices). The naming rule described below does not hold for these (SVG has `clipPath` / `linearGradient` / `feGaussianBlur`); including them would break an invariant the tests hold. `Math` also collides with `System.Math`, since the Razor SDK carries an implicit `using System;`. Curating only the root would just reproduce the same split this decision removed. This exclusion is from the curated `Html` set, not from a mapping altogether: SVG's own element index has a separate static class, `Svg`, sitting outside this group rather than against it (decided 2026-08-21, #319, landed #534). `Svg` reuses the same first-letter-only capitalization rule, keeping the rest of a tag's casing as written (`Svg.ClipPath`, `Svg.LinearGradient`), which stays total over SVG2's 69-element index since none of its names are hyphenated. The one exception is the root `svg` element: `Svg.Svg` is CS0542 (a member cannot share its enclosing type's name), so it is spelled `Svg.Root` instead. `Html` and `Svg` share several tag names (`a` / `audio` / `canvas` / `iframe` / `video`); qualifying the ambiguous spelling (`Html.A` vs. `Svg.A`) resolves it, the same way any other name collision under `using static` does. MathML is out of scope here: its element index contains `annotation-xml`, which breaks the same naming rule's totality, so it needs a naming rule of its own before it can get the same treatment.
...
- The attribute shortcuts are derived by rule (decided 2026-08-20, #490), the same way the curated element set is: a name qualifies when it spells the attribute it emits verbatim, its value is determinate as a single opaque string or a boolean, and it emits exactly one attribute frame. Every entry in the HTML Living Standard's *Index — Attributes* meeting these three traits gets a shortcut — 142 names in total, including the original six (`Href` / `Src` / `Alt` / `Id` / `Type` / `Title`). `class`/`style` are excluded (their own channel, below); `role` is excluded from the derivation (it is ARIA-defined, not a row in the HTML standard's own index) and stays the one hand-picked entry; `data-*`/`aria-*` stay excluded (#244). None of these restrict which element a shortcut may be written on — the boundary this surface checks stays the translation break, not HTML validity (unchanged, #335). `.Class` does not belong to this set: its value is a sequence of classes, and it is the class channel's own spelling (`ARCHITECTURE.md` §2.7(A)). `ARCHITECTURE.md` Appendix B.21 (revisited, #490) has the full argument.
...
The type system only closes the child-position boundary above; it does not close the loop-source one. `ForEach(Rows(_items), …)` compiles — `Rows(_items)` converts to `ForEach`'s `IEnumerable<T>` source parameter without trouble — and reaches the generator, which runs the call at runtime against its design-time-built body: the loop count comes out right, and every yielded item comes out empty. BCF3043 is the backstop for this position, and the same failure at a native `foreach`'s source inside another iterator `[ViewPart]`'s own body, or at a spliced projection's own source.
...
The second argument accepts the same shapes `ForEach`'s content does (§5.3): an inline expression lambda, a block reaching one trailing `return` or degrading through a native `if`/`else` or `switch`, or a matching-arity method group, reread as the call it stands for and answered by the same static-expansion / BCF3030 / Opaque split any other call gets. A delegate held in a variable is still BCF3022: it names no callee a method group could be read as, and it is not a lambda whose syntax could be transplanted either. `.Param` / `.Template` / a component's `.Bind` all follow the same three diagnostics (BCF3005 / BCF3006 / BCF3007) for how a target is selected and for the target parameter.
...
`BlazorCodeFirstGenerator.Initialize` projects each analyzed component down to the subset of `ViewPartRegistry`/`CssScopeRegistry` entries it can actually reach (`ViewPartReachability.Collect`, `ComponentExpansionInput`) before `ComponentModelFactory.Expand` runs on it. Before this stage existed, `registry` and `cssScopeRegistry` were `Combine`d onto `analyses` directly; because Roslyn's `Combine` compares the combined tuple as a unit, a change to either provider's value re-ran `Expand` for every component in the compilation, not only the ones that call the changed view part.

`RegistryBroadcastCostTests` measures the current pipeline against a corpus of N components × 3 view parts. An offset-only edit to one view part file (a leading blank line, which shifts every `TemplateLocation` its `ForEach` body carries but changes no semantics) leaves exactly `(N - callers)` `ComponentModeling` outputs `Cached` at N = 10 / 100 / 1000, where `callers` is the number of components that call the edited view part: `Expand` no longer runs at all for a component whose reachable subset the edit did not touch. The edited view part's own callers still report `Unchanged` in this corpus (`EquatableArray` value equality keeps the generated source itself stable even though their subset changed and `Expand` re-ran to confirm it).

Wall-clock cost for this same edit, measured against a same-shape control edit to an unrelated component file, still grows with N: the projection stage runs once per component on every registry edit regardless of relevance, and the edited view part's own callers still re-expand. Point figures are not published here, for the same reason §7.1 excludes execution time — the delta's variance is large and machine-dependent. Reproduce it with the `RegistryBroadcastCostTests` command in `CONTRIBUTING.md` §Build and test. The measured comparison against the pre-#480 whole-registry shape is recorded in `ARCHITECTURE.md` 付録B.25.
