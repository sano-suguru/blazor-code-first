...
> 設計上の要件: 設計時表現(`Body` / `Chrome`)の override を宣言するコンポーネントクラスは `partial` として宣言する必要があります(Source Generatorがレンダリングメソッドを同一クラスへ生成するため)。非partialクラスはビルドエラー(BC1001)となります。Composeベースを継承するだけで設計時表現を宣言しないクラスには不要です。
