...
- 同じ基準が要素の外にも届きます。イベント名とハンドラの引数型の食い違いがそれで、BCF3028として検査します(2026-08-11決定、#155)。`.On("onclick", (KeyboardEventArgs e) => …)` はコンパイルを通ります。名前と型は食い違ったまま、実行時に壊れます。Razorは同じ形を拒否します。`[EventHandler("onclick", typeof(MouseEventArgs), …)]` という対応表を持っているからです。この表はフレームワークが正準として出荷しているメタデータであり、読んでも誰かが維持し続ける列挙は1つも増えません。したがって上の基準の内側にあり、Razorが行う検査でこの表層がRazorに劣る理由もありません。判定は等価ではなく代入可能性です。`.On("onclick", (EventArgs e) => …)` は正当なままです。対応の無いイベントは報告しません。`Components.Web` を参照しないコンパイルで読めなくなるのはフレームワークの対応表だけであり、作者自身が書いた登録はそこでも読みます(2つの発火形状と読み取り元は付録A A.1)。
...
動的コンテンツ経路(§5.3、Opaque経路)の追加コストは、`RenderFragment` の手書きと同一です(実測)。比較対象は、SSCで組んだ要素に、`[ViewPart]` の付かないメソッドが返す `View`(内部は手書きの `RenderFragment`)を子として1つ加えたコンポーネントと、同じ `RenderFragment` を直接埋め込んだRazorコンポーネントです。上表と同じ2つの境界で一致しました。
