...
- 単項側の最初の対象が、void要素に子を与える形です(BCF3016)。対象はHTML Living Standardのvoid elementsに載る13要素であり、curated集合と同様に規則で定義して列挙を持ちません。2026-08-03の計測(net10.0 / ASP.NET Core 10.0.10)では、`Img["child"]` に対し静的SSRが `<img src="/a.png">child</img>` を出力し、HTMLパーサは子を兄弟のテキストノードへ押し出すため、prerenderでは子が要素の外側に見えます。interactive描画は同じ式に対して `<img>` の子としてテキストノードを付けます。`Br["child"]` は迷子の `</br>` が開始タグとして再解釈されるため `<br>` が1個から2個に増えます。除外第1群の `base` / `link` / `meta` も void であるため、`Element` 経由で子を与えればこの検査の対象です。第1群を除外した理由は子を与えずに書いた場合に無言で不活性であることであり、子を与える形とは別の話です。
