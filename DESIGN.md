...
- 命名規則は、タグ名の先頭一文字を大文字にするだけで、それ以上は整形しないことです。`FigCaption` ではなく `Figcaption`、同様に `Colgroup` / `Optgroup` / `Datalist` / `Textarea` / `Blockquote` / `Fieldset` / `Hgroup` となります。conformingなHTMLタグ名は区切りを持たない全小文字であるため、この規則はcuratedな全ヘルパーに例外なく成り立ちます。成り立つのは第5群を除外しているからです。SVGとMathMLの要素名は全小文字ではなく、これらを入れれば規則も、その上に築いたテストの不変条件も同時に壊れます。したがってヘルパー名とタグ名の対応は全単射です。逆写像は先頭一文字を小文字にすることであり、タグごとに覚える対応はありません(`KnownSymbolsSyncTests` が逆写像の側を固定します)。ヘルパー名を全小文字にして文字面まで一致させる案は、C#の予約語との衝突が0であるという計測の上で退けています(`ARCHITECTURE.md` 付録B.15)。
- 属性とイベントは子と並べるのではなく、タグ直後の装飾チェーンで与えます(`Div.Class("card")["text"]`)。子は続く `[...]` に文字列と `View` を混在させて渡し、生の文字列は暗黙変換でテキストノードになるため専用の `Text()` 構文は持ちません。Blazorの `RenderFragment` もそのまま子になります。この位置取りには、綴りの好み以外の帰結があります(2026-08-09調査、#175)。Lucid / Giraffe.ViewEngine / Falco.Markup は属性を子と同じ値の名前空間に置くため、属性の識別子を `_` の接頭辞か接尾辞で綴り替えています(`class_` / `_class_`)。属性を `.` の後ろへ置くことが、`using static Html` を綴り替え無しの名前で成立させています。これは#73と#87で決着した配置に対する裏付けです。
...
- ミラーには構造的な限界が残ります。いずれもHTMLの側に写す先があって表層にその像が無い形であり、大文字化はこれに当たりません(上記の全単射)。第一に、要素の語彙については、curatedなタグとそれ以外を `Element("…")` で分けるというHTMLには存在しない二分は取り除かれましたが、理由のある6群の除外は残るため、ミラーは全域ではありません。第二に、属性の語彙はそもそも開いています(`data-*` / `aria-*` / 独自属性)。閉じた集合として写せる対象が存在しないため、属性側では `.Attr(name, value)` が汎用経路であり続け、ショートカットを持つ属性とそうでない属性というcurated/汎用の二分は表層に残ります。二分が消えたのは要素の語彙についてのみです。いずれも写像の粒度の問題であり、要素の書き順の問題ではありません。
