...
同一要素に置ける束縛の数は1つに限りません。動機となる形はカスタム要素です。双方向のプロパティを2つ以上持つWeb Componentは、そのそれぞれを `.Bind` で束縛でき、通常の差分検知で正しく描かれます(DOM再同期を要しません)。素のHTMLには利用者が編集できるプロパティを2つ持つ要素がありませんが、それはHTMLについての事実であって、この形を拒否する理由ではありません。この節の原則は両方向に効きます。何も破らない形を検査しないのと同じ理由で、Blazorが受け取って正しく描く形も診断しません。かつてこれを拒否する診断を持っていましたが、根拠が実測で否定されたため撤回しました(`ARCHITECTURE.md` 付録B.5)。
