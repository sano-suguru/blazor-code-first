...
parse に失敗すると、フィールドもDOMも前の値へ戻ります(Blazorの仕様。フィールド側は実測、DOM側は `bind-reject.spec.ts`)。巻き戻しは既に発行している `SetUpdatesAttributeName` が起こしており、この決定が足す発行は1つもありません。帰結として、`oninput` に数値を束縛すると巻き戻しがキーストロークごとに走り、`4.5` と打つと `.` が落ちて `45` になります。イベント名は作者が書くので、`onchange` を選ぶかどうかもその位置にあります。
