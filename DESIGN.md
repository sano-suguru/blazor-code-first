...
ただし「Razorコンパイラの出力と同形式」は成り立ちません。Razorコンパイラは完全に静的な要素サブツリーを単一の `AddMarkupContent` フレームへ畳み込みますが、本方式は要素フレームとテキストフレームを発行します。静的コンテンツが多いコンポーネントでは、Razorのほうがフレーム数が少なく差分対象も小さくなります。上記の測定は全テキストノードを式から供給して両者のフレーム列を一致させた上で行っており、この畳み込み差そのものは最適化機会として別途追跡します。
