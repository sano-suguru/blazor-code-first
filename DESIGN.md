...
- `bool` が届かない名前が1つだけあり、それが `class` です。この名前はクラスチャネルへ畳み込まれ、チャネルは装飾を1つの値へ連結するため、同じ綴りが要素の持つクラス装飾の個数によって二通りに翻訳されます。これは次項の意味での翻訳の破れであり、ここではHTMLパーサではなく生成器自身の畳み込みから生じています。したがって `.Attr("class", bool)` はBCF3023で拒否し、条件付きクラスは文字列側の条件式(`.Class(active ? "on" : null)`)として書きます。値を書かない `.Attr("class")` も同じ規則に届きます。裸の綴りは存在を表しますが、テキストとして連結するチャネルには連結すべきものが無いためです。二通りが具体的に何になるかは付録A BCF3023 が持ちます。
