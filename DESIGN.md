...
The call site is a spread, the same spelling as the declined-key `ForEach` sugar above — not a bare call (`Rows(_items)` alone is BCF1003, the same as any other spread that is not a `[ViewPart]` iterator or a `.Select` projection). `.Key(...)` is optional and is written on the yielded element itself, an ordinary frame decoration, not threaded through a separate `key:` parameter the way `ForEach`'s own key is — a native `foreach` header has no sibling argument to carry one. Omitting it emits no `SetKey`, the same as `ForEach`'s own declined key.
