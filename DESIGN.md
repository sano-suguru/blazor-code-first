...
- `object?` は取りません(2026-08-07決定、#158)。非文字列値の整形は `AddAttribute` の呼び出しの中で起き、フレームには整形済みの文字列が入ります。従うのはその呼び出しを実行したスレッド、すなわち `RenderView` を実行するスレッドのカルチャであり、作者の書くどのコードもそれを決められません(実測。整形がどこで起きるかは2026-08-14に#245で測り直し、`NonStringValueFormattingTests` が固定します)。つまり `object` 経路の出力は呼び出し側から見えない周辺状態に依存し、静的畳み込み(`ARCHITECTURE.md` §2.7(D))も原理的に効きません。`bool` は整形すべきものを持たないため、そのどちらの問題も持ちません。`int` / `DateTime` / enum は呼び出し側で文字列化します(`.Attr("tabindex", index.ToString(CultureInfo.InvariantCulture))`)。カルチャが暗黙の周辺状態ではなく、見える選択として書かれる位置に移ります。再検討には、呼び出し側での文字列化では担えない事例が要ります。Razorが `object` を受けるという観察はその事例ではありません。
- 子として受ける型も `string` と `View` に限り、数値のための綴りは持ちません(2026-08-14決定、#245)。`Div[_n]` は今日CS1503であり、数値は補間文字列か明示的な `ToString` を通ります。ただし前項の根拠はこのチャネルには届きません。整形の時点が両方の綴りで一致するためです(実測)。生成コードは補間を `RenderView` へ構文ごと移し、`__builder.AddContent(seq, $"Count: {_n}")` として発行します。数値の子を許した場合の解決先は `AddContent(int, object?)` の1つで(`int` / `double` / `decimal` / `DateTime` / enum のいずれもここに決まります)、この呼び出しもその場で `ToString` を呼び、`string?` を渡した場合と同じ `Text` フレームへ整形済みの文字列を積みます。静的畳み込みも両者を分けません。数値を補間した時点で文字列が定数でなくなるため `Span[$"n={3}"]` は畳み込まれず、`Span["n=3"]` は畳み込まれます。この非対称は今日すでにあり、数値の綴りを足しても増えも減りもしません。分かれるのは1点だけです。補間には整形をどう行うかを書く場所があり(`$"{_n:F2}"`、`_n.ToString(CultureInfo.InvariantCulture)`)、`Div[_n]` には書き足せるものがありません。つまり足すのは二つ目の綴りであり、既に書ける形との違いは、カルチャを書く場所が無いことだけです。根拠は整形の時点ではなく、整形を書ける位置のほうにあります。Oxpecker.ViewEngineの `IntNode` はエスケープを飛ばすためのノードですが、この面では動機になりません。エスケープは `Text` フレームに対して描画時に一様にかかり、経路が2つ無いためです。ここまでの計測は `ChildValueSpellingTests` と `NonStringValueFormattingTests` が固定します。再検討には、補間と `ToString` では担えない事例が要ります。
