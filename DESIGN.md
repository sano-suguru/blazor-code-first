...
BlazorCodeFirstの中核は、Razorコンパイラと同型のコンパイル戦略です。Razorコンパイラが `.razor` マークアップからC#のレンダリングメソッドを生成するのと同じように、BlazorCodeFirstのSource Generatorは、作者がC#で記述した宣言的な `Body` 式からレンダリングメソッドを生成します。マークアップの代わりにC#の式を「ソース・オブ・トゥルース」とすることで、Razorが実証済みの静的シーケンス割当・差分検知性能をそのまま継承しながら、SwiftUIやJetpack Composeと同等の記述体験を実現します。
...
`Body` に記述された宣言的な式は、Source Generatorによってビルド時にレンダリングメソッドへコンパイルされます。生成コードはRazorコンパイラの出力と同じ形式(静的シーケンス番号付きの `RenderTreeBuilder` 命令列)であるため、Blazorエンジンから見ればBlazorCodeFirstコンポーネントと通常のRazorコンポーネントは区別がつきません。作者が書くコードと実行される命令列の間に、ランタイムの動的解釈や中間ツリーは存在しません。
...
HTML要素にマップしない構文が5つあることは、この表層が系譜に対して5つの語彙を足したという意味ではありません。11のHTML DSL(elm/html / Giraffe.ViewEngine / Falco.Markup / Feliz / Scalatags / kotlinx.html / maud / TSX / Plot / Lucid / TyXML)を読むと、`Fragment` には11すべてに対応物があり(`frag`、`<>`、あるいは裸のリスト)、`Raw` も同様です(`rawText`、`PreEscaped`、`Unsafe`)。`Component<T>()` はBlazorが実体を所有することから来ており、系譜が再利用と呼ぶもの、すなわちマークアップを返す関数に当たるのは `[ViewPart]`(§4.3)の側です。`Slot` はHTML自身の `<slot>` の名前であり、Razorの `ChildContent` にもOxpeckerのブロックにも対応物があります。系譜に無いのは `If` と `ForEach` の2つだけです(2026-08-09調査、#175)。

11のうち9つが自前の `if` / `for` をそのまま子の位置に置けるのは、実行時にツリーを組み立てるため文が置けるからです。10番目のmaudは `@if` / `@for` / `@match` / `@let` を持ちますが、これは自分の文法をパースするproc macroであってライブラリではありません。11番目のTSXだけがこの表層と同じ側にあり、子の位置が式であるため文を置けず、`&&` と三項演算子と `.map` で書きます。この制約を共有する唯一の先例が、専用のコンビネータではなくホスト言語の式演算子で答えているということです。`If` と `ForEach` はその位置に当たり、`.map` に相当する形は #172 が扱います。この表層が文を置けない理由は §1.4 のとおりで、`Body` が式であり、C#に文を子生成の式へ変換する仕組みが無いためです。`Body` を文本体へ広げてネイティブの `if` / `foreach` に完全な静的割当を与える案は、2026-08-11に検討して採りませんでした(#175)。子を渡すチャネルが角括弧である以上、文を置ける場所を作ることは子チャネルをラムダブロックへ差し替えることであり、HTMLが子をタグの内側に書くことの写しでなくなります。そのうえ2つの構文は消えません。素の `foreach` にはキーの置き場所が無く(§4.2のとおりキーは必須です)、式が要る位置には文が書けないためです。2つを消すための案が、消さずに増やします。なお §5.3 のネイティブ制御構文はこの案ではありません。あれは動的リージョンへの縮退であり、正確性は保たれるものの静的割当は得られません。ここで検討して退けたのは、その縮退を無くす向きの案です。再検討には、この2つが実際に学習の障害になっていることの測定が要ります。
...
同一要素に置ける束縛の数は1つに限りません。動機となる形はカスタム要素です。双方向のプロパティを2つ以上持つWeb Componentは、そのそれぞれを `.Bind` で束縛でき、通常の差分検知で正しく描かれます(ただしDOM再同期はカスタム要素には及びません。クライアントが返すのはフォーム要素自身の `value`(チェックボックスなら `checked`)だけで、それ以外の要素には `null` が返るため、修復の働きかける先が存在しません)。素のHTMLには双方向の状態を2つの属性で運ぶ要素がありません(`video` / `audio` の `currentTime` / `volume` はネイティブUIからエンドユーザーが変更できますが、いずれも属性へ反映されないIDLプロパティであり、`.Bind` が使う属性の経路では動かせません)。しかしそれはHTMLについての事実であって、この形を拒否する理由ではありません。この節の原則は両方向に効きます。何も破らない形を検査しないのと同じ理由で、Blazorが受け取って正しく描く形も診断しません。かつてこれを拒否する診断を持っていましたが、根拠が実測で否定されたため撤回しました(`ARCHITECTURE.md` 付録B.5)。
...
UIの部分は `[ViewPart]` 属性を付与した静的メソッドに抽出できます。Source Generatorはこれらを解析対象に含め、実行時に呼び出す代わりに、呼び出しサイトへ静的に展開します。
...
`[ViewPart]` の付かないメソッドが `View` を返す場合、Source Generatorはその内部を解析できないため、当該メソッドは実行時に評価される動的コンテンツとして扱われます(戻り値の `View` に `RenderFragment` を内包させる形式。§5.3)。

属性を要さず、`View` を返す静的メソッドを自動的に展開対象とする案は採っていません(2026-08-11決定、`ARCHITECTURE.md` 付録B.11)。属性は呼び出しサイトごとではなく宣言ごとに1つであり、それが表明しているのは「この宣言は展開されるつもりだ」ということです。表明があるから、展開の契約を満たさない宣言に対してBCF1002が宣言の位置で報告できます。自動適用は、この表明を消す代わりに、契約を満たさない宣言を黙って動的経路へ落とすか、§5.3が意図して残している解析不能な経路の綴りごと拒否するかの、どちらかを選ぶことになります。属性の付け忘れが黙って性能を落とす問題自体は実在しますが、それにはBCF2001の側で答えます(#260)。
...
本方式の要となる4つの変換、すなわち装飾チェーンの畳み込み、`ForEach` のキー整合、`[ViewPart]` の静的インライン展開、静的サブツリーの畳み込みについて、それぞれ「どの入力を、どの生成コードに変えるか」を `ARCHITECTURE.md` §2.7 に入出力例として定義しています。
...
`Body` および `[ViewPart]` メソッド内の要素ヘルパー/装飾/コンビネータの直接記述、および `Component<T>()`・`Fragment`・`Raw` の直接呼び出し(インラインラムダを含む)がSSCの内側であり、完全な静的割当の対象です。SSCの外側は次の2通りに扱われます。
...
解析不能な呼び出し(`[ViewPart]` の付かない `View` 返却メソッド等)は、実行時に評価され、戻り値の `View` に内包された `RenderFragment` がリージョン内で描画されます。この経路のみ通常のヒープ割り当てが発生します。
...
一方、当初この差を「構文位置に固定されたシーケンス番号による」と説明していましたが、実測はそれを支持しません。キーを付けた動的インクリメント方式は、静的割当を一切用いずに本方式と同じ結果(3編集・破棄0)に到達します。`ForEach` はキーを必須とするため、作者が書けるリストは常にこのキー付き領域にあります。このシナリオにおいて本方式が動的インクリメント方式に対して優位を示すのは、キーを持たない場合との比較です。
...
`[ViewPart]` 解析の拡張(ジェネリックヘルパー、ローカル関数対応)と動的リージョンの安定化を進めます。.NET 11 GA(2026年11月)後、Union型・`closed` 階層ベースの `ViewNode` APIをnet11.0ターゲットで正式化します。
