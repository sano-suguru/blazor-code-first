...
- 単項側にはvoid以外のメンバーも計測で見つかっており、いずれも未検査です(#133)。二項側も検査しません(#132)。二項側には書き方の誤りでない形も含まれるため、そちらは診断で直せる対象ですらありません(`Table[Tr[Td["x"]]]` は正しい書き方ですが、パーサが `tbody` を挿入するため二つの経路が食い違います)。計測された残余の一覧と、各項がどちらの側に属するかは両Issueが持ちます。この文書はその列挙を持ちません。
