...
- 単項側にはvoid以外のメンバーも計測で見つかっており、いずれも未検査です(#133)。RCDATA要素(`textarea` / `title`)は内容をテキストとして読むため要素の子が潰れ、`Textarea[Span["x"]]` はprerenderで `value` が `"<span>x</span>"`、interactive描画で `""` になります。raw text要素は `AddContent` のエスケープで壊れ、正しく書く経路は `Raw` です。`textarea` / `pre` は開始タグ直後の改行1個がパーサに落とされます。二項側も検査しません(#132)。`Table[Div["x"]]` は `<div>` がtableの外へ移動し、`P[Div["x"]]` は `<p>` が2個に分割されます。加えて `Table[Tr[Td["x"]]]` は正しい書き方ですがパーサが `tbody` を挿入するため食い違い、`table > tr` のようなセレクタの一致がhydrationの前後で変わります。こちらは書き方の誤りではないため、診断で直せる対象でもありません。
...
- 属性値として受ける型は `string` と `bool` の2つだけです。`bool` はBlazorの条件付き属性の綴りであり、`true` は空値の属性、`false` は属性そのものの省略になります(`disabled` / `checked` / `hidden`)。`object?` は取りません(2026-08-07決定、#158)。非文字列値の整形が従うのは、コンポーネントがフレームを組み立てている時点のカルチャではなく、後に整形するスレッドが持つカルチャです(実測)。したがって `object` 経路の出力は呼び出し側から見えない周辺状態に依存し、静的畳み込み(`ARCHITECTURE.md` §2.7(D))も原理的に効きません。`bool` は整形すべきものを持たないため、そのどちらの問題も持ちません。`int` / `DateTime` / enum は呼び出し側で文字列化します(`.Attr("tabindex", index.ToString(CultureInfo.InvariantCulture))`)。カルチャが暗黙の周辺状態ではなく、見える選択として書かれる位置に移ります。再検討には、呼び出し側での文字列化では担えない事例が要ります。Razorが `object` を受けるという観察はその事例ではありません。`bool` が届かない名前が1つだけあり、それが `class` です。この名前はクラスチャネルへ畳み込まれ、チャネルは装飾を1つの値へ連結するため、`bool` の意味は要素が持つクラス装飾の個数で変わります。1つならそのまま出て `class=""`(クラス一覧の消去)、2つ以上なら連結されて `class="a True"` です(いずれも実測、#159)。同じ綴りが二通りに翻訳されるのは、次項の意味での翻訳の破れであり、ここではHTMLパーサではなく生成器自身の畳み込みから生じています。したがって `.Attr("class", bool)` はBCF3023で拒否し、条件付きクラスは文字列側の条件式(`.Class(active ? "on" : "")`)として書きます。
