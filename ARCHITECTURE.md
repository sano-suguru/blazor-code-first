...
| BCF1003 | Error   | 設計時表現(`Body` / `Chrome`)が静的にシーケンス可能な部分集合へ分類できない。Opaque と Transplantable の経路が入ったため、`View` 返却呼び出しとブロック本体の `ForEach` コンテンツはこの診断ではなく、それぞれ BCF2001 / BCF3030 と BCF3004 が見る。残るのは、呼び出しでも設計時構文でもない式である。保存された `View` の読み出しがその形にあたる。`If` の分岐が生成器の予約名(`__bcf_` 接頭辞と `__builder`)を持つローカルを宣言する形もここへ落ちる。分岐は作者の名前のまま移植されるため、ゲッターと同じ理由で予約名を拒む(#389)。スプライスされた射影(`.. source.Select(item => …)`)の本体が同じ名前を宣言する形も同様である。射影は畳まれたループの内側へ作者の名前のまま移植され、拒否はスプライスの他の形と同じ番号へ落とす(#413) |
...
| BCF3001 | Error   | 現行実装では設計時表現(`BodyComponentBase.Body` または `ChromeLayoutBase.Chrome`)本体内での状態変更(単一方向データフロー違反)。初期検出範囲: コンポーネントインスタンスメンバーへの直接書き込み(代入/複合代入/インクリメント/デクリメント)。遅延ハンドラ引数(入れ子ラムダを含む)内は除外。除外対象はイベントデコレーション(`.OnClick` 等のイベント短縮形と `.On`)のハンドラ引数、`.Bind` のセッター引数、`.Ref` のキャプチャ動作、そしてコンポーネントの `.Param` の値であり、これは名前の列挙ではなく `KnownSymbols` の分類そのものから導かれる。`.Param` の値を除外するのは、そこに書いたデリゲートを呼ぶのが子であってフレーム生成ではないためである(#385)。ハンドラを `EventCallback.Factory.Create` で包んだ綴りも同じく除外し、これは分類ではなく `KnownSymbols.IsEventCallbackFactoryMethod` が答える。`.Bind` のゲッター引数はフレーム生成中に評価されるため除外しない。`.Bind` と `.Param` のセレクタも、値を運ばずパラメータを名指すだけなので除外しない。任意の副作用の完全検出は保証しない。`[ViewPart]` 本体への適用は将来拡張候補 |
...
| BCF3004 | Error   | `ForEach` の `key` がインライン式ラムダでも書かれた `null` でもない、または `content` が生成器の受け付ける形でない。`key` の本体は `SetKey` へ移植されるため式であることを要する。移植先は `RenderView` 自身の走査であるから、本体が生成器の予約名を持つローカルを宣言する形も対象外であり、`content` と同じ走査が答える(#413)。`key` の不在は構文として読む。生成器が移植するのは書かれた本体であって実行時の値ではないため、`null` を保持する変数を渡した形はインライン式ラムダではなくこの診断のままである(#172)。`content` が受け付けるのは3つで、インライン式ラムダ、末尾の `return` を1つだけ持ちローカル宣言文と式文のみを並べたブロック本体ラムダ(§2.3 Transplantable)、単一引数で `View` を返すメソッドグループである。メソッドグループは反復変数を1つ渡す呼び出しとして読み替えられ、他の呼び出しと同じ3分岐(静的展開 / BCF3030 / Opaque)へ落ちる。複数の `return` とネイティブの制御構文はそれぞれ独自のシーケンス空間を要するため対象外。生成器の予約名(`__bcf_` 接頭辞と `__builder`)を持つローカルを宣言する形も、ラムダの本体が式かブロックかを問わず対象外である。どちらも作者の名前のままループの内側へ移植されるためで、ゲッターと同じ走査が答える(#389)。構築済みデリゲート(`new Func<T, View>(M)`)も対象外で、呼び出しサイトで呼び出し先を名指せない |
...
| BCF3009 | Error   | `Element` のタグ引数がコンパイル時定数文字列でない(宣言性・予測可能性のため)か、タグ名の綴りでない。綴りの条件はASCII英字で始まり、以降がASCII英数字・`-`・`_`・`.`(`KnownSymbols.IsValidTagName`)。要素が名乗れない綴りは2つの描画経路が別のものを出し、そのどちらも書いたとおりではない。プリレンダはマークアップへ書き出してHTMLパーサに解釈し直され、interactiveは `createElement` に渡して回路ごと落ちる(#394)。内容モデルの表を要さないためこの検査は §4.1 の境界の内側にある。実行時のタグに経路を与える案は付録B.14 で退けた |
...
| BCF3018 | Error   | getterだけを渡す形の `.Bind` で getter の本体が代入可能でない。許可されるのはメンバーアクセス(`_name` / `_form.Name` / `Model.Items[0].Title`)と要素アクセス(`_dict["k"]`)で、対象が呼び出せる setter を持つこと。呼び出し・演算(`() => _name.ToUpper()`)、get-only プロパティ、`readonly` フィールドは拒否する。setter があることと、導いた代入がそれを呼べることは別の問いなので、`init` アクセサと、生成先から到達できない setter も拒否する。コンポーネント自身の `Body` は同じクラスの partial へ出力されるため到達可否はそのコンポーネント型に対して判定し、`{ get; private set; }` はコンポーネント自身が宣言したものなら許可し、他の型のものは拒否する。`[ViewPart]` の本体は展開先がこの時点で決まっていないため、代わりに setter へのアクセス要件を記録し、到達できない展開先を BCF1002 が拒否する。ローカル変数・パラメータ・`ForEach` の反復変数そのものへの直接代入も拒否する(`Body` はプロパティゲッターでありローカルはレンダリングごとに死ぬため、書き戻しが次のレンダリングに残らない)。反復変数の**メンバー**(`o.Title`)は元の要素を書き換えるので許可する。setter を明示する形へ誘導する。要素とコンポーネントの双方で発火し、同じ形でも引数の個数は面によって違う(要素は3と4、コンポーネントは2と3)ため、形の呼び分けに個数を使わない |
...
| BCF3022 | Error   | `Component<T>().Template` の文脈付きオーバーロード(`Func<TContext, View>` を取る形)の content がインライン式ラムダでない(メソッドグループ/匿名メソッド/ブロック本体ラムダ等)。生成器はシーケンス対象の式と、生成する文脈変数を代入するパラメータシンボルの双方を必要とするため、いずれも取り出せない形は拒否する。位置は content 引数の全体で、書き直す対象が引数の形そのものだからである。引数が0個または2個以上のラムダはこの規則の対象外で、`Func<TContext, View>` へ変換できずC#が先に拒否する。content の本体が生成器の予約名(`__bcf_` 接頭辞と `__builder`)を持つローカルを宣言する形も拒否する。本体は生成するフラグメントの内側へ作者の名前のまま移植されるためで、他の移植位置と同じ走査が答える。文脈変数の改名(`__bcf_context_<数字>` を `__bcf_authored_context_…` へ書き換える規則)が救える名前もここでは拒否する。走査を位置ごとに分けないためであり、改名が救わない名前は実測で壊れている。囲む `ForEach` の反復変数と同じ `__bcf_item_0` を宣言した形が CS0841 になる(#413)。BCF3004 と同じ制約を `ForEach` ではなくテンプレートに置いたもの。番号が BCF3021 を飛ばしているのは、BCF3021 が撤回済み(付録B.5)で再利用しないためである |
...
| BCF3035 | Error   | イベント修飾子(`.PreventDefault` / `.StopPropagation`)が、イベントを書くどの装飾よりも前に書かれている。イベントを書く装飾は `.On`・イベントショートカット・`.Bind` の3つで、どれが直前かは連鎖が決める(§2.7 (A))。修飾子はイベント名を自分では持たないため、直前のイベントに付くという読み以外に解釈が無い。付く先が無いまま出せば、その要素のどのハンドラも応答しない属性名を出力することになり、フレームワークはそれを黙って通す(実測)。位置は装飾名で、作者が動かすか消すかする対象がそこだからである。イベントが要素上に無いとは限らず、連鎖の後方に書かれている場合は修飾子をその後ろへ動かすのが直しであるため、位置はイベントを名指せない。メッセージは装飾名を引数に取る |
| BCF3036 | Error   | 同一のイベントに、同じイベント修飾子が2つ書かれている。対象のイベントは BCF3035 と同じ連鎖の遡りが決めるので、束縛のイベントも同じく対象である。BCF3033 が非属性のフレーム装飾について報告するのと同じ壊れ方だが、別のIDである。3033 の項が対象としているのは属性フレームで「ない」3つであり、この2つは属性フレームだからである(§2.7、実測)。2つの修飾子は別チャネルなので、`.PreventDefault().StopPropagation()` は重複ではない。位置は2つ目の装飾で、消す対象がそちらだからである(BCF3033 と同じ)。メッセージは装飾名とイベント名を引数に取る |
| BCF3038 | Error   | イベント修飾子が、そのイベントの `[EventHandler]` 登録で無効にされている。Blazorは修飾子ごとの可否を `EventHandlerAttribute` が持ち、無効な組み合わせの属性を出しても描画側が黙って無視する。Razorは同じ登録を読んで同じ組み合わせを拒否するので、この検査が無いことはこの一点でRazorに劣ることを意味する(`DESIGN.md` §4.1)。番号はBCF3037の次を取る。3037は#370で廃番であり、番号は再利用しない(`CONTRIBUTING.md` §Conventions the code must uphold、付録B.18)。4引数コンストラクタの並びは `(attributeName, eventArgsType, enableStopPropagation, enablePreventDefault)` で、直感とは逆順である(実測、2026-08-15、`Microsoft.AspNetCore.Components.Web` 10.0.10)。登録は96件あり `preventDefault` は96件すべてで有効、`stopPropagation` を無効にしているのは `oncancel` と `onclose` の2件である。2引数コンストラクタは両方を `false` にするため、作者が自分で登録したカスタムイベントは既定で両方を拒む。これがこの診断を2イベントの規則に留めない経路である。読み取り元と門番はBCF3028 と同一であり、登録の無いイベントは報告しない。同じ名前が食い違う2つの登録を持つ場合、引数型が同じでフラグだけ違う場合も含めて表から落とす(過少報告側が安全)。判定を置く位置はイベント修飾子の対象解決点であり、`.On` 由来と `.Bind` 由来のどちらのイベントも同じ1箇所を通る(§2.7 (A))。位置は修飾子の装飾名で、作者が消す対象がそこだからである(BCF3035 と同じ)。メッセージは装飾名とイベント名を引数に取る |
