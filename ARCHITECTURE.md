...
定数であっても、**文字列でない値は畳みません**(#158)。`AddAttribute` に渡した非文字列値の整形は、コンポーネントがフレームを組み立てている時点のカルチャではなく、後に整形するスレッドが持つカルチャに従うためです。`3.5` が `en-US` で `"3.5"`、`de-DE` で `"3,5"` としてDOMへ届くのに、コンパイラはどちらになるかを知り得ません。畳めば片方が markup へ焼き込まれ、同じ値が「周囲が静的かどうか」で違う文字列になります。除外の代償は畳み込みの取りこぼし1回です。
