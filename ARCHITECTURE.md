...
**Transplantable(構文移植)**: 文が生成コードへ構文ごと移植され、境界リージョンで包まれます(§2.5)。現在の受理範囲は `ForEach` の `content` に書かれたブロック本体ラムダ1つで、その形はローカル宣言文と式文が並び、最後に `return <SSC式>;` が1つ来るものです。移植した文はシーケンス引数を持つ呼び出しを含まないため、content のシーケンス幅は式ラムダの場合と同じです。複数の `return` とネイティブの `if` / `foreach` / `switch` は、それぞれ独自のシーケンス空間を要するのでBCF3004 です。
