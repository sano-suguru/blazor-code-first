...
| BCF3021 | Error   | 同一要素に `.Bind` が2つ以上ある。この表層が1要素につき1つの値だけを束縛すると決めているための拒否であり、Blazorの制約ではない(`SetUpdatesAttributeName` の記録先は要素ではなく直前の属性フレームであり、同一要素の2つの束縛は各々が自分の再同期対象を保つ——実測)。モデルが要素あたり1つの束縛しか持たないのもこの決定に合わせたもの。名前の重複ではないためBCF3010では表せない。この狭め方を維持するかは#162 |
