...
「直前に書かれたイベント」が指すのは `.On` とイベントショートカットだけです。`.Bind` もイベントを書きますが、書き込み先が束縛チャネルであるため修飾子からは見えず、この組み合わせは BCF3037 で拒否します。2つのチャネルは互いの順序を記録しないので、どちらが後に書かれたかを答えられるのは構文だけです。判定はそこを遡り、イベントを生まない装飾を跨いで、最初にイベントを生む装飾が `.Bind` かどうかを見ます。束縛のイベントに修飾子を付けられるようにするには `BindTemplate` に同じ2チャネルを持たせ、束縛のイベントフレームの後ろへ出す必要があり、それは別途の作業です。
...
| BCF3035 | Error   | イベント修飾子(`.PreventDefault` / `.StopPropagation`)が、要素上のどのイベントよりも前に書かれている。修飾子はイベント名を自分では持たないため、直前のイベントに付くという読み以外に解釈が無い。付く先が無いまま出せば、その要素のどのハンドラも応答しない属性名を出力することになり、フレームワークはそれを黙って通す(実測)。位置は装飾名で、作者が動かすか消すかする対象がそこだからである。イベントが要素上に無いとは限らず、連鎖の後方に書かれている場合は修飾子をその後ろへ動かすのが直しであるため、位置はイベントを名指せない。メッセージは装飾名を引数に取る |
| BCF3036 | Error   | 同一のイベントに、同じイベント修飾子が2つ書かれている。BCF3033 が非属性のフレーム装飾について報告するのと同じ壊れ方だが、別のIDである。3033 の項が対象としているのは属性フレームで「ない」3つであり、この2つは属性フレームだからである(§2.7、実測)。2つの修飾子は別チャネルなので、`.PreventDefault().StopPropagation()` は重複ではない。位置は2つ目の装飾で、消す対象がそちらだからである(BCF3033 と同じ)。メッセージは装飾名とイベント名を引数に取る |
| BCF3037 | Error   | イベント修飾子が `.Bind` の後ろに書かれている。`.Bind` もイベントを書くが、書き込み先は束縛チャネルであってイベントチャネルではないため、修飾子からは見えない。放置すると、前方に `.On` があればそちらへ黙って付き、無ければ BCF3035 が「`.On` の後ろへ書け」という、束縛には存在しない直し方を案内する。どちらも呼び出しサイトからは見えないので、解決せず拒否する。判定は連鎖を左へ遡り、イベントを生む最初の装飾が `.Bind` か `.On`/イベントショートカットかで決める。イベントを生まない装飾(`.Class` / `.Attr` / 他の修飾子)は跨ぐ。ノードは答えられない。2つのチャネルは互いの順序を記録しないためであり、順序が残っているのは構文だけである。BCF3035 と別のIDなのは直し方が違うためで、こちらはイベントが在るのに表層が修飾できないという欠落である。Razor は `@bind:event` の隣に `@oninput:preventDefault` を書けるので、規則ではなく未対応として記述する。位置は修飾子で、`.Bind` は正しいコードとして残るからである |
