...
| BCF3013 | Error   | `Component<T>()[…]` で子コンテンツが与えられているが、`T` がそれを受け取れる `ChildContent`(settable な `[Parameter]`、型は `RenderFragment` または `RenderFragment<TContext>`)を持たない。ジェネリックな場合、角括弧は文脈を捨てる外側のラムダを伴って束縛するため対象外である。発行されるものは `.Template` の文脈無視オーバーロードと同じであり、角括弧の中に文脈を読むための名前が無いことがこの読み替えを一意にしている。`ChildContent` 以外の名前を持つジェネリックなフラグメントは角括弧では束縛できず、`.Template` で名指しする |
