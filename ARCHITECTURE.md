...
`RenderExpressionAnalyzer.TryReadTransplantableBlock` がゲッターと `content` と `[ViewPart]` の本体に
答えます。文を置ける
...
**Transplantable(構文移植)**: 文が生成コードへ構文ごと移植され、境界リージョンで包まれます(§2.5)。受理する形は1つで、ローカル宣言文と式文が並び、最後に `return <SSC式>;` が1つ来るブロックです。書ける位置は3つあり、`ForEach` の `content` に書かれたブロック本体ラムダ、設計時表現(`Body` / `Chrome`)のゲッター、そして `[ViewPart]` の本体です。1つ目では文がループの内側へ、残る2つでは `RenderView` のフレーム発行の手前へ落ちます。移植した文はシーケンス引数を持つ呼び出しを含まないため、シーケンス幅は式1つで書いた場合と同じです。複数の `return` とネイティブの `if` / `foreach` / `switch` は、それぞれ独自のシーケンス空間を要するので受理しません。診断は位置で分かれ、`content` はBCF3004、ゲッターはBCF1004、`[ViewPart]` はBCF1002 です。

ブロックが宣言したローカルは、生成器が名前を与えます。展開は2つのメソッド本体を1つのスコープへ潰すため、著者の書いた名前のままでは、著者が入れ子を書いていない位置で同じ名前が2度着地しCS0136 になります。閉じ方は反復変数と同じで、ブロック直下の宣言をスコープ付きレンダー変数として束縛し、宣言子の識別子と参照の双方を穴にして、展開時に `__bcf_local_{ブロックの前順序数}_{索引}` を配ります。前順序数はコンポーネント全体で一意であるため、同じ部品を2回展開しても名前は分かれます。宣言の形は問いません。穴は書かれた名前の位置を置き換えるだけであり、複数の宣言子も分解宣言も `out var` もパターン変数も同じ経路で閉じます。この命名が入るまで `[ViewPart]` の本体が式に限られていたのは、部品の文が呼び出しごとに複製されるからでした(#336)。
...
| BCF1002 | Error   | `[ViewPart]` の静的展開が成立しない。宣言位置では、Source Generatorのサポートする静的展開契約を満たさない場合に報告する。満たさないのは次の形である。拡張メンバー(`DESIGN.md` §4.3、#203)、非静的、ジェネリック、ジェネリック型に含まれる、受理する本体の形でない、`View` も `SlotView` も返さない、`params`・参照渡し・`ElementView` 型のパラメータ、`SlotView` を返さない宣言の `View` 型のパラメータ、生成コードから名指せない型のパラメータ、静的にシーケンス可能でない本体。受理する本体は2つで、式本体と、1つの `return` へ到達するブロックである。その `return` の手前にはローカル宣言文と式文を並べられ、書かれた文は呼び出しサイトのフレーム発行の手前へ移植される(§2.3 Transplantable)。ブロックが宣言したローカルには生成器が名前を与えるため、同じ部品を2度展開しても衝突しない(#336)。受理しないブロックの形はゲッターと同じ4つで、読む実装も1つである。呼び出しサイトでは次の3条件で報告する。(1) 当該メソッドのソース宣言が現コンパイルに無い(メタデータのみ)。定義は現コンパイルの構文から `ForAttributeWithMetadataName` で収集され、ILは本体構文を持たないため、参照先プロジェクトやNuGetパッケージの `[ViewPart]` は常にこれに当たる。(2) 再帰的な展開がサイクルを形成する。(3) 本体が参照する `private` / `protected` メンバーへ展開先から到達できない |
