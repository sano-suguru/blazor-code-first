...
internal static class RenderViewEmitter
{
    ...
    public static SourceText Emit(ComponentModel model)
    {
        ...
        // CS1591: the override below has no doc comment of its own, and GenerateDocumentationFile is on
        // repo-wide (#105) so IDE0005 fires during build -- CS1591 is an unwanted side effect of that,
        // not a documentation gap in a consumer's generated code.
        writer.AppendLine("#pragma warning disable CS0219, CS1591");
        writer.AppendLine();
    ...
    /// <param name="writer">The writer nodes are emitted to.</param>
    /// <param name="node">The node to emit.</param>
    /// <param name="startSeq">The next available sequence number before this node emits.</param>
    /// <param name="key">
    ...
    /// </summary>
    /// <param name="writer">The writer the folded frame, if any, is emitted to.</param>
    /// <param name="run">
    ...
    /// </param>
    /// <param name="seq">The next available sequence number before the fold attempt.</param>
    /// <param name="next">The next available sequence number after this call, whether or not the fold won.</param>
    /// <remarks>
    ...
    /// <summary>
    /// Emits a native `if`/`else` transplanted whole (ARCHITECTURE.md §5.3): one region whose boundary
    /// sequence is fixed to syntactic position, same as <see cref="EmitIf"/>. Unlike <see cref="EmitIf"/>,
    /// every arm's content lands at the SAME sequence number (<paramref name="seq"/> + 1) rather than a
    /// disjoint per-branch range — only one arm ever runs, so there is no collision, and each arm's
    ...
    /// call in <see cref="EmitTransplantedIf"/> owns the one region the whole chain shares; recursing
    /// into a nested <see cref="TransplantedIfNode"/> (an `else if`) must not open a second one.
    /// </summary>
    private static void EmitTransplantedIfArms(IndentedWriter writer, TransplantedIfNode node, int armSeq)
    {
        writer.AppendLine($"if ({node.Condition.ToCode()})");
        writer.AppendLine("{");
        writer.Indent++;
        EmitTransplantedArm(writer, node.Then, armSeq);
        writer.Indent--;
        writer.AppendLine("}");
    ...
    private static void EmitTransplantedArm(IndentedWriter writer, RenderNode arm, int seq)
    {
        ...
        {
            writer.AppendLine(block.Statements.ToCode());
            EmitTransplantedArm(writer, block.Content, seq);
            return;
        }
    ...
    /// above depends on.
    /// </summary>
    /// <param name="bind">The binding whose <c>CreateBinder</c> call is assembled.</param>
    /// <param name="value">
