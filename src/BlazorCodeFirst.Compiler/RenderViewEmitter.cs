...
internal static class RenderViewEmitter
{
    ...
    public static SourceText Emit(ComponentModel model)
    {
        ...
        // CS0219 (as before): an inert composed local can go unread by design.
        // CS8601, CS8620: the framework's own EventCallbackFactoryBinderExtensions.CreateBinder for
        // string annotates its setter Action<string?>, so the inverted-getter binder (`__value =>
        // <value> = __value`) and the explicit-setter cast (`(Action<string>)(<setter>)`) both carry a
        // nullable/non-nullable mismatch against a non-nullable <value> or setter, regardless of the
        // author's own types. Measured against a real dispatch (BindRenderingTests,
        // EmptyInput_DeliversEmptyStringNotNull): an empty text input's oninput never actually delivers
        // null here, so there is no runtime defect to fix — only a static annotation the framework
        // carries defensively. Suppressed for the same reason CS0219 is: generated output is not
        // policed by nullable flow analysis the way hand-written code is.
        writer.AppendLine("#pragma warning disable CS0219, CS8601, CS8620");
        writer.AppendLine();
