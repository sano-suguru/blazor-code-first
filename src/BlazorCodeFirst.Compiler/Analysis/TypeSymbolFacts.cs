...
internal static class TypeSymbolFacts
{
    ...
    /// (<see cref="ExpressionTemplateFactory"/>), and the declared type of a <c>[ViewPart]</c> parameter,
    /// which becomes a local at the expansion site (<see cref="ViewPartDefinitionFactory"/>). Both failures
    ...
    /// generated code, pointing at a line nobody wrote. That is what settles the two arms the view part copy
    ...
    /// view part route validates its methods as non-generic before reaching here, so that arm is unreachable
    ...
    /// <c>default</c>, where the view part copy's <c>default</c> used to mean "nameable". It is left to fall
