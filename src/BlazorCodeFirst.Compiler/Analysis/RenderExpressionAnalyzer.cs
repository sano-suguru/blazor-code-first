...
internal static class RenderExpressionAnalyzer
{
        ...
        IfStatementSyntax ifStatement, ViewPartBodyContext context)
    {
        ...
        // (#361, #569). Registered as a scope for the same reason the lowered `If`'s condition is in
        // ClassifyIf: each branch is normalized against a root of its own, and without this the
...
#pragma warning restore CS8524
        ...
        InvocationExpressionSyntax invocation, ViewPartBodyContext context)
    {
        ...
        if (!ElementTag.TryResolve(tagArg, context, out var tagValue))
        {
            context.Diagnostics.Add(DiagnosticInfo.Create(
    ...
    /// <summary>
    /// <c>.Class</c>/<c>.Attr</c> on a <c>ComponentView&lt;TComponent&gt;</c> receiver: a single
    /// constant HTML attribute on a component call (#314), routed by Blazor into the callee's
    ...
    {
        if (FactoryArguments.Bind(invocation, context) is not { } args
            || args.At(0) is not { } firstArg)
        {
            return null;
        }

        var attributeValue = ValueParameter(method, kind == SurfaceMethodKind.ComponentAttr
            ? SurfaceMethodKind.Attr // reuses .Attr's "name consumes argument 0" arity rule
            : kind);
        // Unlike the element side's ClassifyDecoration, there is no attributeValueType read here:
        // that type only ever feeds FoldIntoClassChannel's admission check (BCF3023), and this
        // receiver has no class channel to admit into — class-channel folding is deliberately not
        // reproduced on ComponentView<T> (#314's scope).
        ...
        if (!TryResolveAttributeValue(args, paramAccess, attributeValue, out var value))
            return null;
        ...
        if (method.ContainingType.TypeArguments.Length == 1
            ...
            && FindSettableParameterIgnoreCase(componentType, attrName, context) is { } collidingParameter)
        {
            context.RejectUnresolvedValueRecovery(invocation.Span);
            context.Diagnostics.Add(DiagnosticInfo.Create(
        ...
        if (HasAttributeBinding(inner, attrName))
        {
            context.RejectUnresolvedValueRecovery(invocation.Span);
            context.Diagnostics.Add(DiagnosticInfo.Create(
                DiagnosticDescriptors.BCF3010, paramAccess.Name.GetLocation(), [attrName]));
            return null;
        }

        return inner with
        {
            Attributes = inner.Attributes.AsImmutableArray().Add(
    ...
    /// this way is re-checked for a rebinding (<see cref="IsRebound"/>) before the walk returns success --
            ...
            switch (current)
            {
                ...
                case InvocationExpressionSyntax invocation:
                    ...
                    if (context.KnownSymbols.IsViewPart(resolvedMethod))
                    {
                        ...
                        {
                            foreach (var peeled in peeledLocals)
                            {
                                if (IsRebound(peeled, sourceExpression, context))
                                    return null;
                            }
                        }
                ...
                case IdentifierNameSyntax identifier:
                    if (context.SemanticModel.GetSymbolInfo(identifier, context.CancellationToken).Symbol
                        ...
                        || peeledLocals?.Contains(local, SymbolEqualityComparer.Default) == true
    ...
    /// declaration enclosing <paramref name="sourceExpression"/>. Called only once
    /// <see cref="ResolveViewPartLoopSource"/> has already matched a <c>[ViewPart]</c> through this local's
    /// initializer, so a rebinding -- legal code that simply discards that call's result before the loop
    /// runs -- must suppress the report rather than mark it as the bug.
    /// </summary>
    /// <remarks>
    ...
    /// <paramref name="sourceExpression"/> stands in for the local's own declaring syntax here -- a local
    /// peeled this far is only ever in scope inside the same member <paramref name="sourceExpression"/>
    /// itself sits in, so the two reach the same <see cref="MemberDeclarationSyntax"/> from either end,
    /// without re-fetching syntax <see cref="ResolveViewPartLoopSource"/> already looked at.
    /// </remarks>
    private static bool IsRebound(ILocalSymbol local, ExpressionSyntax sourceExpression, ViewPartBodyContext context)
    {
        if (sourceExpression.FirstAncestorOrSelf<MemberDeclarationSyntax>() is not { } scope)
            return true;
        ...
        out IfStatementSyntax ifStatement)
    {
        ...
        // `else if`, and the final `else` -- the same shape TryReadTransplantableBlock scans.
        if (DeclaresReservedName(block))
            return false;
